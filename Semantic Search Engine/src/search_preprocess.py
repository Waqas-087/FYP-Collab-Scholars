...
class Indexer:
    ...
    def calc_idf(self, nd):
        ...
        negative_idfs = []
        for word, freq in list(nd.items()):
    ...
    def create_files(self):
        ...
        if self.search_type == "relevance" or self.search_type == "tag" or self.search_type == "title":
            pickle.dump(dictionary, open(f"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_{self.search_type}.pkl", "wb"))
            print("File is ready!")
        else:
...
class UpdateIndexer:
    ...
    def __init__(self, new_corpus, search_type, epsilon=0.25):
        ...
        self.search_type = search_type
        self.prev_search_data = pickle.load(open(f"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_{self.search_type}.pkl", "rb"))
        self.epsilon = epsilon
    ...
    def calc_idf(self):
        ...
        # idf can be negative if word is contained in more than half of documents
        negative_idfs = []
        for word, freq in list(self.prev_search_data['nd'].items()):
    ...
    def dump_file(self):
        pickle.dump(self.prev_search_data, open(f"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_{self.search_type}.pkl", "wb"))
...
class Search:
    ...
    def get_top_n(self, query, documents, n=5):
        ...
        top_n_Score = top_n[:index][::-1]  # Sort in descending order
        return top_n, [documents[i] for i in top_n], scores[top_n_Score]
