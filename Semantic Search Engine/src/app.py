...
def load_corpus_and_data_files():
    ...
    auto_tag = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/svos_file.pkl", "rb"))
    summary = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/summary_file.pkl", "rb"))
    document_file = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/document_file.pkl", "rb"))


def load_search_data_files():
    global search_data_for_relevance, search_data_for_tag, search_data_for_title
    search_data_for_relevance = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_relevance.pkl", "rb"))
    search_data_for_tag = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_tag.pkl", "rb"))
    search_data_for_title = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_title.pkl", "rb"))
...
with app.app_context():
    ...
    data_for_tag = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/tags_pickle.pkl", "rb"))
    data_for_text = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/corpus_file.pkl", "rb"))
    data_for_title = pickle.load(open(r"E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/title_corpus.pkl", "rb"))

    if os.path.exists(os.path.join(os.getcwd(), "E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_relevance.pkl")) == False:
        objj = Indexer(data_for_text, search_type="relevance")
        objj.create_files()

    if os.path.exists(os.path.join(os.getcwd(), "E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_tag.pkl")) == False:
        objj = Indexer(data_for_tag, search_type="tag")
        objj.create_files()

    if os.path.exists(os.path.join(os.getcwd(), "E:\Work\Bahria University\FYP\Intelligent_Document_Finder-master\DataBase/search_file_title.pkl")) == False:
...
def normalize_scores(scores):
    ...
    normalized_scores = []
    for score in scores:
        normalized_score = 100 * (max_score - score) / (max_score - min_score)
...
def search():
    if request.method == 'POST':
        ...
        if applied_filter_type == "Search by Relevance":
            ...
            # Get the results and relevance scores
            indexes, results, relevance_scores = obj.get_top_n(tokenized_query, data, n=10)

            results_titles = []
            results_summaries = []
            results_tags = []

            # Loop over all indexes of resultant documents and create the list of resultant titles,
            # summaries, and tags to pass them on the HTML page.
            for i in indexes:
            ...
            # Loop over the results list and create the list of parts of texts to pass on the HTML page.
            for i in results:
                text_to_show = " ".join(sent_tokenize(i)[:2])  # Get at most 2 sentences from the text of the document to show on the screen
                if text_to_show != '':
                    text.append(text_to_show + '....')
                else:
                    # If we didn't get any text to show, that can be due to a very small document that doesn't
                    # contain any full stop. In that case, append the whole text of that document.
                    text.append(i)
            ...
            for i, relevance_score in enumerate(normalized_scores):
                print(f"Relevance Score_{i+1}: {relevance_score}")

            return render_template('searchbyText.html', text=text, tag=query, title=title, summaries=summaries,
                                   tags=tags,
...
def upload_file():
    global the_exception_text, filename
    if request.method == 'POST':
